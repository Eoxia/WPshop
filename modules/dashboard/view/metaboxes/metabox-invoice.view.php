...
<div class="wps-dashboard-card gridw-3">
	<div class="wps-dashboard-card-header">
		<h3 class="wps-dashboard-card-title">
			<?php esc_html_e( 'Oldest Unpaid Invoices', 'wpshop' ); ?>
	...
	<table class="wps-dashboard-table">
		<thead>
			<tr>
				...
				<th><?php esc_html_e( 'Amount', 'wpshop' ); ?></th>
		...
		<tbody>
			...
			if ( ! empty( $invoices ) ) :
				foreach ( $invoices as $invoice ) : 
					$total_amount += (float) $invoice->data['total_ttc'];
					?>
					<tr>
						<td>
							<a href="<?php echo esc_attr( $dolibarr_url . '/compta/facture/card.php?facid=' . $invoice->data['external_id'] ); ?>">
								<?php echo esc_html( $invoice->data['title'] ); ?>
							</a>
							<i class="fas fa-exclamation-triangle wps-icon-warning"></i>
						</td>
						<td>
							<i class="far fa-building wps-icon-customer"></i>
							<?php if ( ! empty( $invoice->data['third_party']->data['id'] ) ): ?>
								<a href="<?php echo esc_attr( admin_url( 'admin.php?page=wps-third-party&id=' . $invoice->data['third_party']->data['id'] ) ); ?>"><?php echo esc_html( $invoice->data['third_party']->data['title'] ); ?></a>
							<?php else : ?>
								<?php esc_html_e('unknown', 'wpshop' ); ?>
							<?php endif; ?>
						</td>
						<td><?php echo esc_html( number_format( $invoice->data['total_ttc'], 2, ',', ' ' ) ); ?> €</td>
						<td><?php echo esc_html( date( 'd/m/Y', strtotime( $invoice->data['date_invoice'] ) ) ); ?></td>
						<td>
