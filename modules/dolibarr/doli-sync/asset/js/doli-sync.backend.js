...
window.eoxiaJS.wpshop.doliSync.syncEntrySuccess = function( triggeredElement, response ) {
	...
	if ( modal.length > 0 ) {
		...
		var syncContainer = jQuery( triggeredElement ).closest( '.wps-sync-container' );
		if ( syncContainer.length > 0 ) {
			syncContainer.replaceWith( response.data.sync_view );
		} else {
			// Reload the page to reflect the synced changes on the product page
